...
RoboSy is an interactive PowerShell tool built around Windows `robocopy`. It provides a clean numbered menu for moving, copying, permanently deleting, and relocating files or folders while leaving a symbolic link or junction at the original path.

RoboSy is designed for users who want a simple terminal-based workflow without manually writing long `robocopy`, delete, or symbolic-link commands.
...
- Supports drag and drop for path input in normal terminal mode.
- Supports relaunching as Administrator by typing `admin` at prompts.
- Falls back from directory symbolic links to junctions when symlink creation is blocked.
- Prints total elapsed time after each operation.
- Writes daily log files next to the script, with fallback to `%LOCALAPPDATA%\RoboSy\logs`.
...
> Windows blocks drag and drop from Explorer into elevated Administrator terminals.  
> Use normal mode when you need drag and drop.
...
| `.gitignore` | Excludes local logs, editor folders, temporary files, and other non-repository files. |
...
| Path | Reason |
| --- | --- |
| `.Commands/` | Local request notes and working prompts. |
| `logs/` | Runtime logs. |
| `.vscode/` | Local VS Code workspace settings. |
| `.idea/` | Local JetBrains IDE settings. |
| Temporary and backup files | Local editor/system artifacts. |
...
RoboSy is free software released under the [MIT License](LICENSE).

If you copy, modify, publish, distribute, or include substantial parts of RoboSy in another project, keep the copyright and license notice:

```text
RoboSy - Copyright (c) 2026 Kiaro Sama
Licensed under the MIT License.

Original project: RoboSy by Kiaro Sama
Repository: https://github.com/KiaroSama/RoboSy
```

This keeps the project free while preserving attribution.

## Donate

If RoboSy helps you, donations are appreciated.
